...
The following items have been shipped since the initial plan:

- **Agent builder redesign** — Pill-style nodes, library browser panel, hover-delete UX
- **Renamed Workflows to Agents in UI** — All user-facing references now say "Agents" instead of "Workflows"
- **Real-time WebSocket streaming for agent runs** — Live token streaming and status updates during execution
- **4 multi-step agent templates** — Deep Researcher, Code Reviewer, Creative Writer, Data Analyst in `templates/flows/`
- **Fixed template interpolation** — `{{variables}}` in agent/LLM node prompts now resolve correctly
- **Tool parameter aliases for small model compatibility** — Alternate parameter names so smaller models can invoke tools reliably
- **Chat stop button with abort support** — Users can cancel in-progress agent runs from the chat UI
...
| 2026-03-30 | Added "Completed" section for shipped items (agent builder, UI rename, streaming, templates, etc.) |
